...
def test_Pauli():
    ...
    assert (2.5*paulis(p).as_polynomial()).__repr__() == p.__radd__(p).__radd__(0.5*p).__repr__()
    ...
    ### Test trace
    temp = pauli('II').as_monomial()
    temp.c = -1j
    assert (p.trace() == np.trace(p_op)) and (p2.trace() == np.trace(p2_op)) and (temp.trace() == -4j)
...
def test_PauliList():
    ...
    for pcomp2 in p2:
        ...
        temp = pauli('-iII').as_monomial()
        temp.c = -1j
    assert (np.allclose(p.trace(), p_op)) and (np.allclose(p2.trace(), p2_op)) and (temp.trace() == -4j)
...
def test_PauliPolynomial():
    ...
    assert rep.__repr__() == '(0.00-1.00j) ZXZXZ +1 XYIZZ +(0.00+1.00j) YYZII'
